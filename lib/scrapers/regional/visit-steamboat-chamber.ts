...
const SOURCE = "visit-steamboat-chamber";
const RSS_URL = "https://www.steamboatchamber.com/event/rss/";
const TOWN = "Steamboat Springs";
...
function categoryFromRssTag(tags: string[], title: string, venue: string): Category {
  ...
  if (/food|drink|dining|restaurant|brewery|beer|wine/.test(joined)) return "FOOD";
  ...
  if (/festival|holiday|seasonal|winter|summer/.test(joined)) return "SEASONAL";
  if (/sport|fitness|outdoor|hike|trail|bike|ski|snowboard|race|rodeo|hot spring/.test(joined)) return "OUTDOORS";
...
export async function scrapeVisitSteamboatChamber(): Promise<ScraperResult> {
  const errors: string[] = [];
  try {
    ...
    for (const item of items) {
      ...
      if (/\b(conference|convention|seminar|symposium|training|meeting|exposition)\b/i.test(item.title)) continue;
      ...
      const description = cleanDescription(item.descriptionHtml) || `Featured via Steamboat Springs Chamber.`;
      const venueName = "See Steamboat Springs listing";
    ...
    if (events.length === 0) {
      errors.push("visit-steamboat-chamber: RSS feed returned no future-dated items");
    }
    ...
    return {
      source: SOURCE,
      events: [],
      errors: [`visit-steamboat-chamber: ${error instanceof Error ? error.message : "Unknown error"}`],
    };
  }
}
