...
const SOURCE = "visit-estes-park";
const RSS_URL = "https://www.visitestespark.com/event/rss/";
const TOWN = "Estes Park";
...
/**
 * Visit Estes Park uses the same Simpleview CMS as VisitDenver + VisitGolden,
 * so the same /event/rss/ escape hatch works. See visit-denver.ts for the full
 * rationale on parsing dates out of the description CDATA (pubDate is an
 * expiry marker, not the event start time).
 */
...
function categoryFromRssTag(tags: string[], title: string, venue: string): Category {
  ...
  if (/sport|fitness|outdoor|hike|trail|recreation|rmnp|national park/.test(joined)) return "OUTDOORS";
...
export async function scrapeVisitEstesPark(): Promise<ScraperResult> {
  const errors: string[] = [];
  try {
    ...
    for (const item of items) {
      // Same Simpleview convention/meeting leak as VisitDenver.
      ...
      const description = cleanDescription(item.descriptionHtml) || `Featured via Visit Estes Park.`;
      const venueName = "See Visit Estes Park listing";
    ...
    if (events.length === 0) {
      errors.push("visit-estes-park: RSS feed returned no future-dated items");
    }
    ...
    return {
      ...
      events: [],
      errors: [
        `visit-estes-park: ${error instanceof Error ? error.message : "Unknown error"}`,
      ],
    };
  }
}
